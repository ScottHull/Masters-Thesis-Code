...
ax8_1.plot(time_list_ma[0:core_formation_max_time_index + 1], temperatures[0:core_formation_max_time_index + 1],
           linewidth=2.0, color='black', label='Temperature at CMB')
ax8_2.plot(time_list_ma[0:core_formation_max_time_index + 1], pressures[0:core_formation_max_time_index + 1],
           linewidth=2.0, color='black', label='Pressure at CMB')
...
ax9_1.grid()
ax9_1.set_title("Metal-Silicate Partitioning at Vesta CMB Over Time")
...
# ax34_1.plot(fO2, [log10(cmbd_fO2[index][0]) for index, i in enumerate(fO2)], linewidth=2.0, label='Time $t=0$')
# ax34_1.plot(fO2, [log10(cmbd_fO2[index][-1]) for index, i in enumerate(fO2)], linewidth=2.0, label='Time $t=5$ Ma')
ax34_1.fill_between(fO2, [log10(cmbd_fO2[index][0]) for index, i in enumerate(fO2)],
                    [log10(cmbd_fO2[index][-1]) for index, i in enumerate(fO2)], color='green', label="$t=0-5$ Ma")
ax34_1.axvspan(-2.45, -2.25, color='blue', alpha=0.2, label='Reducing Model')
ax34_1.axvspan(-1.10, -0.8, color='red', alpha=0.2, label='Oxidizing Model')
ax34_1.grid()
ax34_1.set_title("Metal-Silicate Partitioning at Vesta CMB")
...
ax35_1.plot(list(reversed(fO2)), list(reversed(initial_hf182_conc)), linewidth=2.0, color='black', linestyle="-", label="$^{182}$W (Bulk)")
ax35_1.plot(list(reversed(fO2)), list(reversed(initial_conc_w184)), linewidth=2.0, color='red', linestyle="-", label="$^{184}$W (Bulk)")
ax35_1.plot(fO2, [i[-1] for i in bulk_core_conc_w182_list], color='black', linestyle="--", linewidth=2.0, label='$^{182}$W (Core)')
ax35_1.plot(fO2, [i[-1] for i in bulk_core_conc_w184_list], color='red', linestyle="--", linewidth=2.0, label='$^{184}$W (Core)')
...
ax35_1.axvspan(-1.10, -0.8, color='red', alpha=0.2, label='Oxidizing Model')
ax35_1.set_title("Inferred Concentrations in Vesta")
ax35_1.set_xlabel("$fO_2$ ($\Delta$IW)")
ax35_1.set_ylabel("Concentration (ppb)")
ax35_1.grid()
ax35_1.legend(loc='top right')

fig36 = plt.figure()
ax36_1 = fig36.add_subplot(111)
ax36_1.plot(list(reversed(fO2)), [i / j for i, j in zip(list(reversed(initial_hf182_conc)), list(reversed(initial_conc_w184)))],
            linewidth=2.0, color='black', linestyle="-", label="$^{182}$W/$^{184}$W (Bulk)")
ax36_1.plot(fO2, [k / l for k, l in zip([i[-1] for i in bulk_core_conc_w182_list], [i[-1] for i in bulk_core_conc_w184_list])],
            color='black', linestyle="--", linewidth=2.0, label='$^{182}$W/$^{184}$W (Core)')
ax36_1.plot(fO2, [k / l for k, l in zip([i[-1] for i in bulk_mantle_conc_w182_list], [i[-1] for i in bulk_mantle_conc_w184_list])],
            color='black', linestyle="-.", linewidth=2.0, label='$^{182}$W/$^{184}$W (Mantle)')
...
ax36_1.axvspan(-1.10, -0.8, color='red', alpha=0.2, label='Oxidizing Model')
ax36_1.set_title("$^{182}$W/$^{184}$W in Vesta")
ax36_1.set_xlabel("$fO_2$ ($\Delta$IW)")
ax36_1.set_ylabel("$^{182}$W/$^{184}$W")
ax36_1.grid()
ax36_1.legend(loc='top right')
# ax36_1.set_yscale('log')

fig37 = plt.figure()
ax37_0 = fig37.add_subplot(111)
ax37_1 = fig37.add_subplot(211)
ax37_2 = fig37.add_subplot(212)
ax37_1.plot(fO2, [i[-1] for i in epsilon_mantle], color='black', linestyle="-", linewidth=2.0, label="Mantle")
ax37_1.plot(fO2, [i[-1] for i in epsilon_bulk], color='black', linestyle="--", linewidth=2.0, label="Bulk")
ax37_2.plot(fO2, [i[-1] for i in epsilon_core], color='black', linestyle="-", linewidth=2.0, label="Core")
ax37_1.axvspan(-2.45, -2.25, color='blue', alpha=0.2, label='Reducing Model')
ax37_1.axvspan(-1.10, -0.8, color='red', alpha=0.2, label='Oxidizing Model')
ax37_2.axvspan(-2.45, -2.25, color='blue', alpha=0.2, label='Reducing Model')
ax37_2.axvspan(-1.10, -0.8, color='red', alpha=0.2, label='Oxidizing Model')
ax37_0.spines['top'].set_color('none')
ax37_0.spines['bottom'].set_color('none')
ax37_0.spines['left'].set_color('none')
ax37_0.spines['right'].set_color('none')
ax37_0.tick_params(labelcolor='w', top='off', bottom='off', left='off', right='off')
ax37_0.xaxis.labelpad = 20
ax37_0.yaxis.labelpad = 20
ax37_0.set_title("$\epsilon$$^{182}$W in Vesta (Present Day)")
ax37_0.set_xlabel("$fO_2$ ($\Delta$IW)")
ax37_0.set_ylabel("$\epsilon$$^{182}$W")
ax37_1.grid()
ax37_2.grid()
