...
ax36_1.plot(list(reversed(fO2)), [i / j for i, j in zip(list(reversed(initial_hf182_conc)), list(reversed(initial_conc_w184)))],
            linewidth=2.0, color='red', linestyle="-", label="$^{182}$W/$^{184}$W (Bulk)")
...
ax37_1.plot(fO2, [i[-1] for i in epsilon_mantle], color='green', linestyle="-", linewidth=2.0, label="Mantle")
ax37_1.plot(fO2, [i[-1] for i in epsilon_bulk], color='red', linestyle="-", linewidth=2.0, label="Bulk")
ax37_2.plot(fO2, [i[-1] for i in epsilon_core], color='blue', linestyle="-", linewidth=2.0, label="Core")
